...
class AGTaichiMPM:
    ...
    def saveState(self, file_name):
        print('[AGTaichiMPM] saving state to ' + file_name)
...
while True:
    ...
    for i in range(100):
        ...
        if time >= agTaichiMPM.max_time:
            agTaichiMPM.saveState('result.h5')
            print('[AGTaichiMPM2D] simulation done.')
            exit(0)
    gui.circles(agTaichiMPM.particle_x.to_numpy(), radius=2, color=0xFFFFFF)
