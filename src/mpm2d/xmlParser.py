...
import numpy as np
import xml.etree.ElementTree as ETco
...
class MPMXMLData:
    def __init__(self, file_name):
        print('[AGTaichiMPM2D] Parsing xml file: ' + str(file_name))
        tree = ET.parse(file_name)
        root = tree.getroot()
        if root.tag != 'AGTaichiMPM2D':
            print('[AGTaichiMPM2D] Could not find root note AGTaichiMPM2D. Exiting...')
            exit(-1)
